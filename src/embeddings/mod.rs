...
impl CandleProvider {
    ...
    pub fn load(model_id: &str, dimension: usize) -> Result<Self> {
        // Metal GPU produces inconsistent vector distances with candle 0.9
        // (cosine scores out of [0,1] range, CPU/Metal vectors incompatible).
        // Keeping CPU until candle Metal matures. Re-evaluate on candle 1.0+.
